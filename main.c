...
void printHelp(FILE *out, SNAKEVAL val)
{
  ...
  {
    uint64_t *closure_ptr = (uint64_t *)(val - CLOSURE_TAG);
    uint64_t arity = *closure_ptr;
    uint64_t *ptr = (closure_ptr + 1);
    uint64_t num_free_vars = *(closure_ptr + 2);
    fprintf(out, "[%p - 5] ==> <function with arity=%ld, ptr=%p, num_free_vars=%ld, free_vars=[", (uint64_t *)val, arity, ptr, num_free_vars);
    for (int i = 0; i < num_free_vars; i++)
    {
      uint64_t free_var = *(closure_ptr + 3 + i);
      printHelp(out, free_var);
      fprintf(out, ", ");
    }
    fprintf(out, "]>");
  }
  else if ((val & FORWARDING_TAG_MASK) == FORWARDING_TAG)
  {
    fprintf(out, "forwarding to ");
    fflush(out);
    fprintf(out, "%p", (int *)(val - FORWARDING_TAG));
    fflush(out);
    return;
  }
...
void error(uint64_t code, SNAKEVAL val)
{
  ...
  case ERR_GET_NOT_NUM:
    fprintf(stderr, "Error: get index is not a number, got ");
    printHelp(stderr, val);
    break;
  case ERR_SET_NOT_NUM:
    fprintf(stderr, "Error: set index is not a number, got ");
    printHelp(stderr, val);
    break;
  case ERR_TUPLE_DESTRUCTURE_MISMATCH:
